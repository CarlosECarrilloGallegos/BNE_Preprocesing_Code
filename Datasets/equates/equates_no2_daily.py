...
for j in time:
    ...
    for i in csvs:
        ...
        equates = equates[['longitude','latitude','date','NO2_AVG']]
    
        equates_year.append(equates)
    
    equates_final = pd.concat(equates_year)
    
    equates_final = equates_final.rename(columns={'longitude':'lon','latitude':'lat','NO2_AVG':'equates'})
    ...
    equates_final.to_csv('/data0/shr/bne/data/nox/daily/base_models/formatted/equates/equates_no2_' + j + '_daily.csv', index=False)
